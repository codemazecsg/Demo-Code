...
if exists (select * from sys.tables t inner join sys.schemas s on t.schema_id = s.schema_id where t.name = 'Fact_Policies' and s.name = 'dbo')
	drop table [dbo].[Fact_Policies];
GO

CREATE TABLE [dbo].[Fact_Policies]
(
	[Id] [int] NOT NULL,
	[ExtId] [int] NOT NULL,
